...
int GetFDFromName(char *name){
    ...
    if(i == 50)    return -1;   //file nasel tr getfdname incorrect error yeil
    else        return i;   //file cha nav aasel tr i
}
...
int CreateDILB(char *name,int permission){
    ...
    while(temp != NULL){  //empty inode shodyn satthi
        if(temp->FileType == 0)
        break;
        temp = temp->next;
    }   

    while(i<50){  //empty innode .....shodto aah yecht
        if(UFDTArr[i].ptrfiletable == NULL)
            break;
        i++;
    }

    UFDTArr[i].ptrfiletable = (PFILETABLE)malloc(sizeof(FILETABLE)); //filetable chi memory allocate zali aah
    ...
    UFDTArr[i].ptrfiletable->ptrinode = temp; //300
...
int WriteFile(int fd,char *arr,int isize){
    if(((UFDTArr[fd].ptrfiletable->mode) != WRITE) && ((UFDTArr[fd].ptrfiletable->mode) != READ+WRITE))
            return -1;   //writefile nma e int main return -1 bhyachi

    if(((UFDTArr[fd].ptrfiletable->ptrinode->permission) != WRITE) && ((UFDTArr[fd].ptrfiletable->ptrinode->permission) != READ+WRITE))
            return -1; 

    if((UFDTArr[fd].ptrfiletable->writeoffset) == MAXFILESIZE)
            return -2; //tr offset maxfilesize zali tr lihun nahi shkt

    if((UFDTArr[fd].ptrfiletable->ptrinode->FileType) != REGULAR)
            return -3;   //

    strncpy((UFDTArr[fd].ptrfiletable->ptrinode->Buffer) + (UFDTArr[fd].ptrfiletable->writeoffset),arr,isize);
                                        //        1000+5(writeoffset)= 1005
    (UFDTArr[fd].ptrfiletable->writeoffset) = (UFDTArr[fd].ptrfiletable->writeoffset) + isize;

    (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) = (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) + isize;
                                //fileactualsize  = fileactaulsize + isize;
    return isize;
}
